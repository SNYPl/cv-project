...
const Experience = ({ setPage }) => {
  const [formCount, setFormCount] = useState([<ExperienceForm />]);
  ...
  return (
    <div className={style.educationContainer}>
      ...
      <div className={style.addBtn}>
        <button
          onClick={() =>
            setFormCount((state) => [...state, <ExperienceForm />])
          }
        >
      ...
      <div className={style.nextBtn}>
        <button onClick={() => setPage("person")}>უკან</button>
        <button onClick={() => setPage("education")}>შემდეგი</button>
      </div>
    </div>
  );
};
